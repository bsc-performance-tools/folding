...
string gnuplotGenerator::gnuplot_addresses_observed (
	InstanceGroup *ig,
	const ObjectSelection *os,
	const string & prefix,
	const string & TimeUnit,
...
)
{
	...
	gplot << "set size 1,0.175;" << endl
	      << "set origin 0,0.80;" << endl
	      << "set bmargin 0; set lmargin 14; set rmargin 17;" << endl
	      << "unset key;" << endl
	      << "unset xtics;" << endl;
	...
	gplot << "unset y2tics;" << endl
	      << "set ytics nomirror format '%0" << numhexdigits_maxaddress << "x' (" << minaddress_stack;
	for (int i = 1; i <= 4; i++)
		gplot << ", " << minaddress_stack+i*((maxaddress_stack-minaddress_stack)/4);
	gplot << ");" << endl
	  << "set yrange [" << minaddress_stack << ":" << maxaddress_stack << "];" << endl
	  << "set y2range [" << minaddress_stack << ":" << maxaddress_stack << "];" << endl;
	...
	gplot << "set y2tics nomirror format '%0" << numhexdigits_maxaddress << "x' (" << minaddress_nonstack;
	for (int i = 1; i <= 4; i++)
		gplot << ", " << minaddress_nonstack+i*((maxaddress_nonstack-minaddress_nonstack)/4);
	gplot << ");" << endl << 
...
)
{
	...
	gplot << fixed << setprecision(2) <<
	  ...
	  "# set term x11 size 800,600;" << endl <<
	  "set term wxt size 800,600;" << endl << endl <<
	  "set multiplot layout 2,1 title \"" << os->toString (true) <<
	    " - " << groupName <<  " - " << regionName << "\"" << endl << 
	  "set datafile separator \";\";" << endl <<
	  "set nokey; " << endl <<
	  "set x2range [0:1];" << endl <<
	  "set yrange [0:*];" << endl <<
	...
	gplot << "set label 'Performance counter rate (in Mevents/s)' at screen 0.025, screen 0.5 rotate by -90 center;" << endl
	  << "set label 'Addresses referenced' at screen 0.975, screen 0.5 rotate by -90 center;" << endl;

	gplot << "set size 1, " << StackPlotProportion << ";" << endl <<
	  "set origin 0, " << 1-StackPlotProportion << ";" << endl << 
	  "set xrange [0:X_LIMIT*1./FACTOR];" << endl <<
	  "set tmargin 4; set bmargin 0; set lmargin 10;" << endl;

	gplot << "set y2tics nomirror format '%0" << numhexdigits_maxaddress << "x' (" << minaddress_stack;
	for (int i = 1; i <= 5; i++)
	...
	gplot << "set size 1, " << 1-StackPlotProportion << ";" << endl <<
	...
	gplot << ");" << endl << 
	  ...
	  "set xlabel 'Time (in ms)';" << endl <<
	  "unset x2tics;" << endl << 
	  "set tmargin 0; set bmargin 6; set lmargin 10;" << endl << endl;
